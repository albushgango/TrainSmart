<script>
	...
	let gefiltert = $derived(filtereUebungen(uebungSuche, alleUebungenZeigen ? '' : finalerSubtyp));
	...
	function uebungMitSets(u) {
		const sets =
			Array.isArray(u.sets) && u.sets.length > 0
				? u.sets
		...
		return {
			...
			sets: sets.map((set, idx) => ({
				nr: idx + 1,
				wiederholungen: Math.max(
					1,
					Math.round(zahlOderFallback(set.wiederholungen, u.wiederholungen || 8))
				),
	...
	function uebungMitZusammenfassung(u) {
		...
		const referenzSet = relevanteSets.reduce(
			(bestes, set) => (Number(set.gewicht) >= Number(bestes.gewicht) ? set : bestes),
			relevanteSets[0]
		);
		...
		return {
			...
			wiederholungen: Math.max(
				1,
				Math.round(zahlOderFallback(referenzSet?.wiederholungen, mitSets.wiederholungen || 8))
			),
	...
	function satzAktualisieren(uebungIdx, satzIdx, feld, wert) {
		...
		uebungen = uebungen.map((u, i) => {
			...
			const sets = mitSets.sets.map((set, j) =>
				j === satzIdx ? { ...set, [feld]: neuerWert } : set
			);
	...
	let liveDauerMin = $derived(
		liveStartZeit
			? Math.max(1, Math.round((liveNow - liveStartZeit) / 60000))
			: Number(dauerInput) || 0
	);
	...
	>
		{#if kamVonEmpfehlung}
			<div class="coach-hinweis">
				...
				<p>
					Sport und Split sind bereits vorausgewählt. Du kannst alles anpassen, bevor du speicherst.
				</p>
			</div>
		{/if}
		...
		<div class="form-grid">
									...
									<div class="lauf-vorschau">
										{#if istLaufen && livePace}
											<div class="lv-item">
												...
												<span class="lv-wert"
													>{livePace.formatted} <span class="lv-einheit">min/km</span></span
												>
											</div>
										{/if}
										{#if liveSpeed}
			...
			<section class="details-panel" aria-label="Trainingsinhalt">
				...
				{#if sport === 'Kraft'}
								...
								<div class="rest-timer" aria-live="polite">
									...
									<div class="rest-info">
										...
										<div class="rest-actions">
											...
											<button type="button" class="skip" onclick={pauseSkippen}
												>Pause skippen</button
											>
											<button type="button" onclick={() => pauseAendern(30)}>+30s</button>
										</div>
									</div>
					...
					<div class="trainings-grid">
						<div class="vorschlag-spalte">
							{#if workoutVorschlag.length > 0}
								<div class="workout-vorschlag">
									...
									{#if bearbeiteterVorschlag.length > 0}
										<div class="wv-uebungen">
											{#each bearbeiteterVorschlag as name}
												<span
													class="wv-chip"
													class:uebernommen={uebungen.some((u) => u.name === name)}
												>
													<span>{name}</span>
													<button
														type="button"
									...
									{:else}
										<p class="wv-empty">
											Noch keine Übungen im Vorschlag. Füge unten eine Übung hinzu.
										</p>
									{/if}
											...
											<div class="wv-add-combobox">
												...
												{#if vorschlagDropdownOffen}
													<div class="wv-dropdown">
														{#each Object.entries(vorschlagGefiltert) as [gruppe, namen]}
															...
															{#each namen as n}
																<button
																	type="button"
																	class="wv-option"
																	disabled={bearbeiteterVorschlag.some(
														...
														{#if vorschlagSuche.trim() && !bearbeiteterVorschlag.some((v) => v.toLowerCase() === vorschlagSuche
																		.trim()
																		.toLowerCase())}
															<button
																type="button"
									...
									<p class="wv-hinweis">
										Entferne oder ergänze Übungen zuerst im Vorschlag. Standard: 2 Sätze × 8
										Wiederholungen. Gewicht, Wiederholungen und Sätze kannst du unten anpassen.
									</p>
								</div>
							{/if}
						</div>
						...
						<div class="uebungen-spalte">
															...
															<div class="live-set" class:erledigt={set.erledigt}>
																...
																<div class="live-set-field">
																	...
																	<button
																		...
																		onclick={() =>
																			liveEditorOeffnen(idx, satzIdx, 'wiederholungen')}
																...
																{#if istLiveEditorAktiv(idx, satzIdx, 'gewicht') || istLiveEditorAktiv(idx, satzIdx, 'wiederholungen')}
																	<div class="live-stepper-editor">
																		<div class="live-stepper-top">
																			...
																			<button type="button" onclick={liveEditorSchliessen}
																				>Schliessen</button
																			>
																		</div>
																			...
																			<button
																				...
																				onclick={() =>
																					liveWertAnpassen(
																						idx,
																						satzIdx,
																						aktiverLiveEditor.feld,
																						-liveEditorBasisSchritt(aktiverLiveEditor.feld)
																					)}
																				aria-label="{liveEditorLabel(
																					aktiverLiveEditor.feld
																				)} reduzieren">-</button
																			>
																			<strong>
													...
													<div class="ue-edit-grid">
														<label>
															...
															<input
																...
																value={u.saetze}
																oninput={(e) =>
																	uebungAktualisieren(idx, 'saetze', e.currentTarget.value)}
															/>
														</label>
														<label>
															<span>Wdh.</span>
															<input
																...
																oninput={(e) =>
																	uebungAktualisieren(idx, 'wiederholungen', e.currentTarget.value)}
															/>
														</label>
														<label>
															<span>kg</span>
															<input
																...
																value={u.gewicht}
																oninput={(e) =>
																	uebungAktualisieren(idx, 'gewicht', e.currentTarget.value)}
															/>
														</label>
													</div>
												{/if}
							...
							{#if !neueUebungAktiv}
								<button type="button" class="ue-add-btn" onclick={() => (neueUebungAktiv = true)}>
									+ Übung hinzufügen
								</button>
							{:else}
								<div class="ue-form">
									...
									<div class="ue-form-aktionen">
										<button type="button" class="ue-speichern" onclick={uebungSpeichern}
											>Hinzufügen</button
										>
										<button type="button" class="ue-abbrechen" onclick={neueUebungAbbrechen}
											>Abbrechen</button
										>
									</div>
								</div>
				...
				<div class="abschluss-panel">
					<label for="notiz" class="field-label"
						>Notiz <span class="optional">(optional)</span></label
					>
