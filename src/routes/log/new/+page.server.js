...
export const actions = {
	default: async ({ request }) => {
		...
		if (!AKTIVE_SPORTARTEN.includes(sport)) {
			return fail(400, {
				error: 'Rad und Schwimmen werden später ergänzt. Bitte wähle aktuell Kraft oder Laufen.'
			});
		}
		...
		const distanz =
			distanzRaw && Number.isFinite(Number(distanzRaw)) ? Number(distanzRaw) : undefined;
		const avgHr = avgHrRaw && Number.isFinite(Number(avgHrRaw)) ? Number(avgHrRaw) : undefined;
		const hoehenmeter =
			hoehenmeterRaw && Number.isFinite(Number(hoehenmeterRaw))
				? Number(hoehenmeterRaw)
				: undefined;
				...
				if (Array.isArray(eingegebene) && eingegebene.length > 0) {
					const docs = eingegebene
						.map((u) => {
							...
							const schwerstesSet = referenzSets.reduce(
								(bestes, set) => (set.gewicht >= bestes.gewicht ? set : bestes),
								referenzSets[0]
							);
							const liveNotiz =
								erledigteSets.length > 0
							...
							return {
								...
								wiederholungen:
									Number(schwerstesSet?.wiederholungen) || Number(u.wiederholungen) || 1,
