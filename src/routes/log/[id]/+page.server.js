...
export async function load({ params }) {
	...
	const uebungenMitVergleich = await Promise.all(
		uebungen.map(async (u) => {
			...
			if (
				vorherige &&
				vorherige.sessionId?.datum &&
				new Date(vorherige.sessionId.datum) < new Date(session.datum)
			) {
				vergleich = {
	...
	const alleNamen = [...new Set([...ALLE_UEBUNGEN.map((u) => u.name), ...distinctNamen])].sort();
...
export const actions = {
	...
	update: async ({ request, params }) => {
		...
		const hoehenmeter =
			hoehenmeterRaw && Number.isFinite(Number(hoehenmeterRaw)) ? Number(hoehenmeterRaw) : null;
	...
	uebungHinzufuegen: async ({ request, params }) => {
		...
		if (
			!name ||
			!Number.isFinite(saetze) ||
			!Number.isFinite(wiederholungen) ||
			!Number.isFinite(gewicht)
		) {
