<script>
	...
	let gefiltert = $derived(filtereUebungen(neuName, neuAlleZeigen ? '' : sessionSubtyp));
	...
	let sichtbareSportarten = $derived(
		AKTIVE_SPORTARTEN.includes(sport)
			? AKTIVE_SPORTARTEN
			: [sport, ...AKTIVE_SPORTARTEN].filter(Boolean)
	);
	...
	function splitFazit(splits) {
		...
		if (unterschied > 8) {
			return {
				titel: 'Negativer Split',
				text: `Du wurdest zum Ende ca. ${Math.round(unterschied)}s/km schneller.`
			};
		}
		if (unterschied < -8) {
			return {
				titel: 'Schneller Start',
				text: `Du bist ca. ${Math.abs(Math.round(unterschied))}s/km langsamer geworden.`
			};
		}
...
<div class="page" style="--card-farbe: {sportFarbe[session.sport]}">
	...
	{#if !bearbeitenAktiv}
		...
		<div class="aktionen">
			<button class="btn-edit" onclick={() => (bearbeitenAktiv = true)}> ✎ Bearbeiten </button>
			<button class="btn-delete" onclick={() => (loeschenBestaetigung = true)}> 🗑 Löschen </button>
		</div>
		...
		{#if session.laufDaten && session.laufDaten.verlauf && session.laufDaten.verlauf.length > 0}
			...
			{#if splits && splits.length > 0}
				{@const besterSplit = splits.reduce(
					(best, split) => (split.dauerSek < best.dauerSek ? split : best),
					splits[0]
				)}
				...
				<section class="lauf-analyse">
					...
					<div class="splits-tabelle">
						{#each splits as split}
							{@const pct =
								((langsamstePace - split.dauerSek) / (langsamstePace - schnellstePace || 1)) * 100}
				...
				<section class="lauf-analyse intervall-card">
					...
					{:else}
						<p class="intervall-empty">
							Dieser Lauf wirkt gleichmässig oder die GPS-Daten sind zu grob für eine sichere
							Intervall-Erkennung.
						</p>
					{/if}
				</section>
							...
							<div class="hr-zone-zeile">
								<div class="hr-zone-kopf">
									<div class="hr-zone-titel">
										<span class="hr-zone-badge" style="background: var({zone.cssVar});"
											>Z{zone.zone}</span
										>
								...
								<div class="hr-zone-balken-bg">
									<div
										class="hr-zone-balken"
										style="width: {Math.max(
											zone.sekunden > 0 ? 4 : 0,
											zone.prozent
										)}%; background: var({zone.cssVar});"
									></div>
								</div>
								<div class="hr-zone-meta">
									<span
										>{zone.von > 0
											? `${zone.von}${zone.bis ? '-' + zone.bis : '+'}`
											: `bis ${zone.bis}`} bpm</span
									>
			...
			{#if hrDaten.length >= 2}
				...
				<section class="lauf-analyse">
					...
					<svg
						viewBox="-10 -10 300 120"
						preserveAspectRatio="xMidYMid meet"
						width="100%"
						class="lauf-chart"
					>
						<path
							d={`${hrPath} L ${chartB} ${chartH} L 0 ${chartH} Z`}
							fill="var(--sport-kraft)"
							opacity="0.1"
						/>
						<path
							d={hrPath}
							fill="none"
							stroke="var(--sport-kraft)"
							stroke-width="2"
			...
			{#if paceDaten.length >= 2}
				...
				<section class="lauf-analyse">
					<div class="la-header">
						<h2>Pace-Verlauf</h2>
						<span class="la-sub"
							>{formatPaceSekunden(paceMin)}–{formatPaceSekunden(paceMax)} min/km · oben = schneller</span
						>
					</div>
					...
					>
						...
						{/each}
						<path
							d={`${pacePath} L ${chartB} ${chartH} L 0 ${chartH} Z`}
							fill="var(--accent)"
							opacity="0.1"
						/>
						<path
			...
			{#if hoeheDaten.length >= 2}
				...
				<section class="lauf-analyse">
					...
					>
						<path
							d={`${hPath} L ${chartB} ${chartH} L 0 ${chartH} Z`}
							fill="var(--sport-rad)"
							opacity="0.18"
						/>
						<path
			...
			<section class="uebungen-section">
					...
					<div class="uebungen-liste">
						{#each uebungen as uebung (uebung._id)}
							{#if editUebungId === uebung._id}
								...
								<form method="POST" action="?/uebungAktualisieren" use:enhance class="uebung-form">
									...
									<div class="uebung-form-aktionen">
										...
										<button
											type="button"
											class="btn-uebung-cancel"
											onclick={() => (editUebungId = null)}>Abbrechen</button
										>
										<button
											type="button"
											class="btn-uebung-delete"
								...
								>
									<div class="uebung-zeile">
										...
										{#if uebung.vergleich}
											...
											<span class="vergleich" class:plus={diff > 0} class:minus={diff < 0}>
									...
									<div class="uebung-werte-readonly">
										<span class="werte-text">
											...
											<strong>{uebung.gewicht} kg</strong>
										</span>
										{#if uebung.vergleich}
											<span class="letzte"
												>letztes Mal: {uebung.vergleich.saetze}×{uebung.vergleich.wiederholungen} @ {uebung
													.vergleich.gewicht}kg</span
											>
										{/if}
									</div>
									{#if uebung.notiz}
				...
				{#if neueUebungAktiv}
					...
					>
							...
							{#if neuDropdownOffen}
								<div class="ue-dropdown-detail">
									...
									{#if Object.keys(gefiltert).length === 0}
										<div class="ue-leer">Keine Treffer — wird als eigene Übung gespeichert</div>
									{/if}
								</div>
							{/if}
						</div>
						...
						<div class="uebung-werte">
							<div class="wert-feld">
								<input type="number" name="saetze" min="1" max="20" placeholder="4" required />
								<span class="wert-label">Sätze</span>
							</div>
							<div class="wert-feld">
								<input
	...
	{:else}
		...
		<form method="POST" action="?/update" use:enhance>
			...
			<input type="number" id="dauer" name="dauer" min="1" max="600" bind:value={dauer} required />
			...
			<textarea id="notiz" name="notiz" bind:value={notiz} placeholder="Wie war das Training?"
			></textarea>

			<div class="form-aktionen">
				<button type="submit" class="btn-save">Speichern</button>
				<button type="button" class="btn-cancel" onclick={bearbeitenAbbrechen}> Abbrechen </button>
			</div>
		</form>
	{/if}
</div>
...
{#if loeschenBestaetigung}
	<div class="dialog-backdrop" onclick={() => (loeschenBestaetigung = false)} role="presentation">
		<div
			class="dialog"
		...
		>
			...
			<p>
				Diese Aktion kann nicht rückgängig gemacht werden. Alle Übungen werden ebenfalls gelöscht.
			</p>
...
{#if uebungLoeschId}
	<div class="dialog-backdrop" onclick={() => (uebungLoeschId = null)} role="presentation">
		<div
			class="dialog"
			onclick={(e) => e.stopPropagation()}
			role="dialog"
			aria-modal="true"
			aria-labelledby="dialog-uebung-title"
		>
