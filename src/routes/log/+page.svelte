	...
	<header>
		...
		<div class="header-aktionen">
			<a
				href="/log/import"
				class="btn-export"
				aria-label="Garmin TCX importieren"
				...
				>
					...
					<div class="card-body">
						<span class="sport">
							...
							{session.sport}{#if session.subtyp}<span class="subtyp-badge">{session.subtyp}</span
								>{/if}
						</span>
					...
					<div class="card-right">
						{#if session.distanz}
							...
							<span class="rpe">{session.dauer} min · RPE {session.rpe}</span>
						{:else}
