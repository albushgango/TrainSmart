...
<div class="page">
	...
	{:else}
		...
		<div class="tabs" role="tablist">
			<button
				class="tab"
				class:aktiv={aktiverTab === 'uebersicht'}
				role="tab"
				aria-selected={aktiverTab === 'uebersicht'}
				onclick={() => (aktiverTab = 'uebersicht')}>Übersicht</button
			>
			<button
				class="tab"
				class:aktiv={aktiverTab === 'aktivitaet'}
				role="tab"
				aria-selected={aktiverTab === 'aktivitaet'}
				onclick={() => (aktiverTab = 'aktivitaet')}>Aktivität</button
			>
			<button
				class="tab"
				class:aktiv={aktiverTab === 'fortschritt'}
				role="tab"
				aria-selected={aktiverTab === 'fortschritt'}
					...
					<div class="sport-liste">
						{#each sportVerteilung as sport}
							...
							<div class="sport-item">
								<span class="sport-dot" style="background: {sportFarbe[sport.sport] ?? '#888'};"
								></span>
				...
				<section class="chart-section">
					<div class="section-header">
						...
						<span class="section-sub"
							>{recordsProSport[sportFilter].sessionsAnzahl} {sportFilter}-Sessions</span
						>
					</div>
					...
					<div class="records-grid">
						<div class="record-card">
							...
							<span class="record-wert"
								>{recordsProSport[sportFilter].laengsteSession.dauer} min</span
							>
							<span class="record-meta"
								>{formatKurzes(recordsProSport[sportFilter].laengsteSession.datum)}</span
							>
						</div>
						<div class="record-card">
							...
							<span class="record-meta"
								>{formatKurzes(recordsProSport[sportFilter].hoechstesRpe.datum)}</span
							>
						</div>
					</div>
				</section>
				...
				<section class="chart-section">
					...
					{#if uebungsFortschritt.length === 0}
						<div class="chart-card empty-fortschritt">
							...
							<p class="sub">
								Logge eine Kraft-Session und füge Übungen hinzu, um deinen Fortschritt zu sehen.
							</p>
						</div>
					{:else}
										...
										{#if spark}
											...
											>
												{#if spark.einzelpunkt}
													<circle
														cx={spark.einzelpunkt.x}
														cy={spark.einzelpunkt.y}
														r="2.5"
														fill="var(--accent)"
													/>
												{:else}
			...
			{:else if sportFilter === 'Laufen'}
				...
				{#if !laufFortschritt}
					<div class="chart-card empty-fortschritt">
						...
						<p class="sub">
							Logge eine Laufsession mit Distanz, dann erscheinen hier deine Pace-, Distanz- und
							HR-Verläufe.
						</p>
					</div>
				{:else}
						...
						<div class="records-grid lauf-records">
							{#if laufFortschritt.records.laengsteDistanz}
								<div class="record-card">
									...
									<span class="record-wert"
										>{laufFortschritt.records.laengsteDistanz.distanz} km</span
									>
									<span class="record-meta"
										>{formatKurzes(laufFortschritt.records.laengsteDistanz.datum)}</span
									>
								</div>
							{/if}
							{#if laufFortschritt.records.schnellstePace}
								<div class="record-card">
									...
									>
									<span class="record-meta">
										{laufFortschritt.records.schnellstePace.distanz}km · {formatKurzes(
											laufFortschritt.records.schnellstePace.datum
										)}
									</span>
								</div>
							{/if}
							{#if laufFortschritt.records.hoechsteHr}
								<div class="record-card">
									...
									<span class="record-wert"
										>{laufFortschritt.records.hoechsteHr.bpm}
										<span class="rec-einh">bpm</span></span
									>
									<span class="record-meta"
										>{formatKurzes(laufFortschritt.records.hoechsteHr.datum)}</span
									>
								</div>
							{/if}
							{#if laufFortschritt.records.meisteHm}
								<div class="record-card">
									...
									<span class="record-wert"
										>{laufFortschritt.records.meisteHm.meter} <span class="rec-einh">m</span></span
									>
									<span class="record-meta"
										>{formatKurzes(laufFortschritt.records.meisteHm.datum)}</span
									>
								</div>
							{/if}
						</div>
					...
					{#if laufFortschritt.verlauf.length >= 2}
						...
						{#if paceDaten.length >= 2}
							<section class="chart-section">
								...
								<div class="chart-card">
									...
									>
										<path
											d={`${paceChart.path} L 280 100 L 0 100 Z`}
											fill="var(--accent)"
											opacity="0.08"
										/>
										<path
									...
									<div class="chart-legende">
										<span
											>{paceDaten.length} Sessions · Schnellste {laufFortschritt.records
												.schnellstePace?.formatted ?? '—'}</span
										>
									</div>
								</div>
							</section>
						...
						<section class="chart-section">
							<div class="section-header">
								...
								<span class="section-sub"
									>Total {distanzDaten.reduce((sum, d) => sum + d.wert, 0).toFixed(1)} km</span
								>
							</div>
							...
							<div class="chart-card">
								...
								>
									<path
										d={`${distanzChart.path} L 280 100 L 0 100 Z`}
										fill="var(--sport-laufen)"
										opacity="0.08"
									/>
									<path
										d={distanzChart.path}
										fill="none"
										stroke="var(--sport-laufen)"
										stroke-width="2"
										stroke-linecap="round"
										stroke-linejoin="round"
									/>
