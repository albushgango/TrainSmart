...
export async function load() {
	...
	SPORTARTEN.forEach((sport) => {
		...
		const laengste = sessionsDesSports.reduce(
			(max, s) => (!max || s.dauer > max.dauer ? s : max),
			null
		);
		const hoechstesRpe = sessionsDesSports.reduce(
			(max, s) => (!max || s.rpe > max.rpe ? s : max),
			null
		);
	...
	const alleUebungen = await Uebung.find().populate({ path: 'sessionId', select: 'datum' }).lean();
	...
	const uebungsFortschritt = [...fortschrittMap.values()]
		.map((u) => {
	...
	const laufSessions = alleSessions.filter(
		(s) => s.sport === 'Laufen' && Number.isFinite(s.distanz) && s.distanz > 0
	);

	let laufFortschritt = null;
	if (laufSessions.length > 0) {
		// Records
		const laengsteDistanz = laufSessions.reduce(
			(max, s) => (!max || s.distanz > max.distanz ? s : max),
			null
		);
		...
		const hrAnalysen = laufSessions
			.map((s) =>
				s.laufDaten?.hrZonen?.zonen?.length
					? s.laufDaten.hrZonen
