<script>
	let { data } = $props();
	const { empfehlung, letzteSessions, wochenziel, wochenTage, wochenSessions } = data;
	...
	let gewaehlterTag = $state(wochenTage.find((t) => t.istHeute)?.datum ?? wochenTage[0].datum);
	...
	<div class="dashboard-grid">
			...
			<div class="empf-content">
				...
				{#if kannTrainingVorbereiten}
					<a href={empfehlungHref} class="empf-aktion">
						<span
							>{empfehlung.naechsterTag
								? `${empfehlung.naechsterTag} vorbereiten`
								: 'Training vorbereiten'}</span
						>
		...
		<section class="wochenziel-card">
			...
			<p class="wz-text">
				...
				{:else}
					Noch {wochenziel.ziel - wochenziel.erreicht}
					{wochenziel.ziel - wochenziel.erreicht === 1 ? 'Session' : 'Sessions'} bis zum Ziel
				{/if}
			</p>
		</section>
						...
						>
							...
							<div class="card-body">
								<span class="sport">
									...
									{session.sport}{#if session.subtyp}<span class="subtyp-badge"
											>{session.subtyp}</span
										>{/if}
								</span>
